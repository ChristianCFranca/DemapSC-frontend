...
export default new Vuex.Store({
  ...
  mutations: {
    ENGAGE_TOKEN_COUNTDOWN(state) {
      ...
      state.tokenRemainingSeconds =
        24 * 60 * 60 -
        Math.trunc((new Date().getTime() - originalTokenTime) / 1000); // 24*60 minutos menos o tempo passado
      setInterval(function() {
        if (state.tokenRemainingSeconds) state.tokenRemainingSeconds--;
      }, 1000);
    },
    ...
    SET_MATERIAIS_DIVERSOS(state, materiais) {
      state.materiaisDiversosList = materiais;
    },
    SET_TODOS_OS_PEDIDOS(state, pedidos) {
      // Ordenação por data
      if (pedidos) state.pedidos = pedidos.sort((a, b) => b.number - a.number);
    },
    USER_CLEAR_DATA() {
    ...
    SET_CURRENT_QUANTITATIVOS_EMPRESA(state, quantitativos) {
      state.quantitativos.empresa = quantitativos;
    },
    SET_CURRENT_QUANTITATIVOS_CARTAO_CORP(state, quantitativos) {
      state.quantitativos.cartaoCorp = quantitativos;
    },
  ...
  actions: {
    authenticate({ commit }, credentials) {
      return apiClient.post("/auth/token", credentials).then((response) => {
    ...
    requestNewPassword(_, username) {
      return apiClient
        .get(`/auth/users/password/request?username=${username}`)
        .then((response) => response);
    },
    register(_, newUserData) {
      return apiClient
        .post("/auth/users/create/", newUserData)
        .then((response) => response);
    },
    changePassword({ getters }, userDataJSON) {
      userDataJSON.username = getters.getEmail;
      return apiClient
        .put("/auth/users/update/password", userDataJSON)
        .then((response) => response);
    },
    updateUser(_, userDataJSON) {
      return apiClient
        .put("/auth/users/update/general", userDataJSON)
        .then((response) => response);
    },
    deleteUser(_, username) {
      return apiClient
        .delete(`/auth/users/delete/?username=${username}`)
        .then((response) => response);
    },
    getUsers({ commit }) {
      return apiClient.get("/auth/users/").then((response) => {
        commit("SET_ALL_USERS", response.data);
      });
    },
    getMateriais({ commit }, empresa) {
      return apiClient
        .get(`/crud/materiais/?empresa=${empresa}`)
        .then((response) => {
          commit("SET_MATERIAIS", response.data);
        });
    },
    getMateriaisDiversos({ commit }, searchString) {
      return apiClient
        .get(`/crud/materiais/diversos/?search_string=${searchString}`)
        .then((response) => {
          commit("SET_MATERIAIS_DIVERSOS", response.data);
        });
    },
    getEmpresas({ commit }) {
      return apiClient.get("/empresas/").then((response) => {
    ...
    getTodosOsPedidos({ commit, getters }) {
      return apiClient
        .get(
          `/crud/pedidos/${
            getters.getCurrentUserEmpresas
              ? `?empresa=${getters.getCurrentUserEmpresas}`
              : ``
          }`
        )
        .then((response) => {
    ...
    postNovoPedido(_, novoPedido) {
      return apiClient.post("/crud/pedidos/", novoPedido).then((response) => {
        return response;
      });
    },
    getPedidoPDFs(_, pdfs_ids) {
      return apiClient.post("/collect-data/pdfs", pdfs_ids).then((response) => {
        return response;
      });
    },
    getQuantitativos({ commit, dispatch }, dataForQuantitativo) {
      commit("UNSET_CURRENT_QUANTITATIVOS");
      if (dataForQuantitativo.tipo == 1)
        return apiClient
          .get(
            `/collect-data/quantitativosEmpresa?empresa=${dataForQuantitativo.empresa}&mes=${dataForQuantitativo.mes}&ano=${dataForQuantitativo.ano}`
          )
          .then((response) => {
            commit("SET_SNACKBAR", {
              message: "Quantitativos obtidos com sucesso",
              color: "success",
            });
            commit("SET_CURRENT_QUANTITATIVOS_EMPRESA", response.data);
          })
          .catch((error) => {
            console.log(error);
            if (error?.response?.status === 401) {
              commit("SET_SNACKBAR", {
                message: "Usuário não autenticado ou não possui permissão",
                color: "error",
              });
              dispatch("logout");
      ...
      else if (dataForQuantitativo.tipo == 2)
        return apiClient
          .get(
            `/collect-data/quantitativosCartao?mes=${dataForQuantitativo.mes}&ano=${dataForQuantitativo.ano}`
          )
          .then((response) => {
            commit("SET_SNACKBAR", {
              message: "Quantitativos obtidos com sucesso",
            ...
            commit("SET_CURRENT_QUANTITATIVOS_CARTAO_CORP", response.data);
          })
          .catch((error) => {
            console.log(error);
            if (error?.response?.status === 401) {
        ...
        commit("SET_SNACKBAR", {
          message: "Nenhuma opção foi selecionada para a busca",
          color: "error",
        });
    },
    reupdatePedidoPDFs({ getters, dispatch }) {
      ...
      return apiClient
        .put(`/collect-data/redo-pdfs/${pedido["_id"]}`, pedido)
        .then((response) => {
    ...
    updateCurrentPedidoDireto({ commit, getters }, temp_idx) {
      ...
      return apiClient.put(`/crud/pedidos/direct/${_id}`, pedido).then(() => {
    ...
    updateCurrentPedido({ getters, commit, dispatch }) {
      ...
      if (pedido.statusStep === 2) {
        ...
        for (let idx = 0; idx < pedido.items.length; idx++) {
          if (
            pedido.items[idx].valorTotal !== null &&
            pedido.items[idx].aprovadoAssistente
          )
            // Tem que ter valor diferente de 0 e estar aprovado pelo assistente
        ...
        if (
          pedido.items.every(
            (item) => !item.aprovadoAssistente || item.categoria === "Fixo"
          )
        ) {
          // Corrige para itens fixos
        ...
        for (let idx = 0; idx < pedido.items.length; idx++) {
          if (
            pedido.items[idx].valorTotal !== null &&
            pedido.items[idx].aprovadoFiscal
          )
            // Tem que ter valor diferente de 0 e estar aprovado pelo fiscal
            valorDaSolicitacao += pedido.items[idx].valorTotal;
        }
        pedido.valorDaSolicitacao = valorDaSolicitacao; // Atualizamos o valor total da proposta
        if (
          pedido.items.every(
            (item) => !item.aprovadoFiscal || item.categoria === "Fixo"
          )
        ) {
          // Corrige para itens fixos
          pedido.status = "Aguardando aquisição dos itens";
    ...
    finishCurrentPedido({ state, getters, commit, dispatch }, idx) {
      ...
      if (pedido.items.every((item) => item.recebido || !item.aprovadoFiscal)) {
        ...
        for (let idx = 0; idx < pedido.items.length; idx++) {
          if (
            pedido.items[idx].valorGasto !== null && // Tem que ter valor diferente de null
            !pedido.items[idx].almoxarifadoPossui && // Almoxarifado NÃO deve possuir
            pedido.items[idx].aprovadoFiscal
          )
            // Fiscal deve ter aprovado
      ...
      return apiClient
        .put(`/crud/pedidos/${_id}?email=${email}`, pedido)
        .then(() => {
    ...
    collectData(_, rota) {
      return apiClient
        .get(`/collect-data/${rota}`, { responseType: "arraybuffer" })
        .then((response) => {
          return response;
        });
    },
    getFaturamento(_, faturamentoInfo) {
      return apiClient
        .post(`/faturamentos`, faturamentoInfo)
        .then((response) => {
          return response;
        });
    },
  ...
  getters: {
    ...
    getFirstLastName: (state) =>
      `${state.currentUser.nome.split(" ")[0]} ${
        state.currentUser.nome.split(" ").slice(-1)[0]
      }`,
    ...
    getAllUsers: (state, getters) =>
      state.allUsers.length === 0
        ...
        : state.allUsers.filter(
            (obj) =>
              state.permissionsPerRole[getters.getRole].includes(obj.role) &&
              obj.username !== getters.getEmail
          ),
    getCurrentEmpresaQuantitativos: (state) => state.quantitativos.empresa,
    getCurrentCartaoCorpQuantitativos: (state) =>
      state.quantitativos.cartaoCorp,
    getSnackbar: (state) => state.snackbar,
    getMateriais: (state) => state.materiaisList,
    getMateriaisDiversos: (state) => state.materiaisDiversosList,
    getMateriaisList: (state) =>
      state.materiaisList.length === 0
        ? []
        : [...state.materiaisList.map((item) => item["descricao"])],
    getMateriaisDiversosList: (state) =>
      state.materiaisList.length === 0
        ? []
        : [...state.materiaisDiversosList.map((item) => item["descricao"])],
    getCanUserDownload: (state, getters) =>
      state.rolesThatCanDownload.includes(getters.getRole),
    getPedidosForCurrentUser: (state) =>
      state.pedidos.filter((obj) => obj["email"] === state.currentUser.email),
    getPedidos: (state) => state.pedidos,
    getPedidosAtivos: (state) =>
      state.pedidos.filter((obj) => obj.active && obj.statusStep !== 6),
    getPedidosPendentes: (_, getters) =>
      getters.getPedidosAtivos.filter((pedido) =>
        getters.getPseudoApprovalsForRoles[pedido.statusStep].includes(
          getters.getRole
        )
      ),
    getPedidosPendentesSimiao: (_, getters) =>
      getters.getPedidosAtivos.filter(
        (pedido) =>
          pedido.statusStep === 5 &&
          pedido.active &&
          pedido.items.some(
            (item) =>
              item.direcionamentoDeCompra === "Demap" &&
              !item.almoxarifadoPossui &&
              !item.recebido
          )
      ),
    getPedidosLiberadosAlmox: (state) =>
      state.pedidos.filter((obj) => obj["statusStep"] >= 5 && obj["active"] && obj.items.some(
            (item) =>
              item.almoxarifadoPossui
          )),
    ...
    getPedidosConcluidos: (state) =>
      state.pedidos.filter((obj) => obj["statusStep"] === 6),
  },
});
